...
**Назначение документа**: Это полная документация по мониторингу и отладке Voice Bot Pipeline. Документ описывает систему логирования, метрики производительности, отладку проблем и устранение неполадок.

**Для кого**: Документ предназначен для разработчиков, отлаживающих проблемы, для системных администраторов, мониторящих работу системы, и для пользователей, сталкивающихся с ошибками.

**Что содержит**:
- Мониторинг (все метрики: время обработки, использование API, качество извлечения)
- Отладка (инструкции по отладке, использование логов, инструменты отладки)
- Логи (форматы логов, уровни логирования, структура записей)
- Устранение неполадок (типичные проблемы и решения, диагностика ошибок)
- Метрики SAG v2.0 (расширенные метрики обработки, статистика экстракторов)
- Профилирование производительности (инструменты профилирования, оптимизация)
- Мониторинг в production (best practices для production окружения)

**Когда использовать**: Используйте этот документ при возникновении ошибок, при оптимизации производительности, при настройке мониторинга, или когда нужно понять что происходит в системе.
